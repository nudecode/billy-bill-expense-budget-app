<div>
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <button wire:click="openAddIncomeModal" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 text-white text-[13px] font-semibold rounded-lg hover:bg-green-700 transition-colors">
            + Add Income
        </button>
    </div>
    {{-- Summary card --}}
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-2 sm:px-5 py-3.5">
                            <div class="flex items-center gap-1 justify-end lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                <button wire:click="openEditOneOffIncome({{ $income->id }})"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 hover:bg-slate-50 hover:text-slate-700 transition-all"
                                        title="Edit">
                                    <i class="fa-regular fa-pen-to-square text-sm"></i>
                                </button>
                                <button wire:click="confirmDeleteOneOffIncome({{ $income->id }})"
        ...
        <div class="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl z-10 max-h-[90vh] overflow-y-auto"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <div class="flex items-center gap-2.5">
                    ...
                    <h2 class="text-[15px] font-bold text-slate-900">{{ $editingOneOffIncomeId ? 'Edit Income' : 'Add Income' }}</h2>
                </div>
                <button wire:click="closeIncomeModal()" class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-all">
            ...
            <div class="px-5 py-4 space-y-4">
                ...
                <div>
                    ...
                    <input wire:model="incomeName" type="text" placeholder="e.g. Salary"
                           class="w-full px-3 py-2 border border-slate-200 rounded-lg text-[13.5px] focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 transition-all @error('incomeName') border-red-400 @enderror">
                ...
                <div class="grid {{ $isRecurringIncome ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-2' }} gap-3">
                    ...
                    <div>
                        ...
                        <select wire:model="incomeFrequencyId"
                                class="w-full px-3 py-2 border border-slate-200 rounded-lg text-[13.5px] bg-white focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 transition-all @error('incomeFrequencyId') border-red-400 @enderror">
            ...
            <div class="flex items-center justify-end gap-3 px-5 py-4 border-t border-slate-100">
                <button wire:click="closeIncomeModal()" type="button"
                        class="px-4 py-2 text-[13px] font-semibold text-slate-600 hover:bg-slate-100 rounded-lg transition-all">
                    Cancel
                </button>
                <button wire:click="saveIncome()" wire:loading.attr="disabled" wire:target="saveIncome"
                    ...
                    <span wire:loading.remove wire:target="saveIncome">Save</span>
                    <span wire:loading wire:target="saveIncome" class="flex items-center gap-1.5"><i class="fa-solid fa-spinner fa-spin text-xs"></i> Saving…</span>
                </button>
            </div>
        </div>
    </div>
    ...
    @if($showDeleteOneOffIncomeModal)
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
         x-data="{ open: false }"
         x-init="$nextTick(() => open = true)"
         @keydown.escape.window="$wire.cancelDeleteOneOffIncome()">

        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"
             x-show="open" x-transition:enter="transition ease-out duration-200"
             x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
             @click="$wire.cancelDeleteOneOffIncome()"></div>
        ...
        <div class="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl z-10"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <div class="flex items-center gap-2.5">
                    ...
                    <h2 class="text-[15px] font-bold text-slate-900">Delete Income</h2>
                </div>
                <button wire:click="cancelDeleteOneOffIncome()" class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 transition-all">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 px-5 py-4 border-t border-slate-100">
                <button wire:click="cancelDeleteOneOffIncome()" type="button"
                        class="px-4 py-2 text-[13px] font-semibold text-slate-600 hover:bg-slate-100 rounded-lg transition-all">
                    Cancel
                </button>
                <button wire:click="deleteOneOffIncome()" wire:loading.attr="disabled" wire:target="deleteOneOffIncome"
                    ...
                    <span wire:loading.remove wire:target="deleteOneOffIncome">Delete</span>
                    <span wire:loading wire:target="deleteOneOffIncome" class="flex items-center gap-1.5"><i class="fa-solid fa-spinner fa-spin text-xs"></i> Deleting…</span>
                </button>
            </div>
        </div>
    </div>
